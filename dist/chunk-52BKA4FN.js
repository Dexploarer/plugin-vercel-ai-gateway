...
var _GatewayProvider = class _GatewayProvider {
  ...
  */
  async generateTextSmallStreaming(runtime, params) {
    const { StreamingGatewayProvider } = await import("./streaming-gateway-provider-IMRK6YG6.js");
    ...
    const result = await pRetry(async () => {
      const response = await embed({
        model: {
          ...
          doEmbed: /* @__PURE__ */ __name(async ({ values, headers, abortSignal }) => {
            const apiKey = getApiKey(this.runtime);
            const baseURL = getBaseURL(this.runtime);
            const response2 = await fetch(`${baseURL}/embeddings`, {
              method: "POST",
              headers: {
