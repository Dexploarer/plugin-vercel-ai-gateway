...
var _GatewayProvider = class _GatewayProvider {
  ...
  */
  async generateTextSmallStreaming(runtime, params) {
    const { StreamingGatewayProvider } = await import("./streaming-gateway-provider-VOP3EGF5.js");
  ...
  */
  async generateEmbedding(params) {
    ...
    const result = await pRetry(async () => {
      ...
      const { createOpenAI } = await import("./dist-GAC5ZV3D.js");
      const provider = createOpenAI({
        apiKey: getApiKey(this.runtime),
        baseURL: getBaseURL(this.runtime)
      });
      const modelName = modelToUse.replace(/^openai\//, "");
      logger3.info(`[AIGateway] Embedding request - Model: ${modelName}, Text: "${params.text}"`);
