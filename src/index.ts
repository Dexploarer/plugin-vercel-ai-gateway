import { IAgentRuntime, ModelType, logger, Route } from "@elizaos/core";
...
const plugin = {
  name: "aigateway",
  description:
    "Universal AI Gateway integration with Grok model protection and streaming support",
  ...
  async init(runtime: IAgentRuntime) {
    ...
    logger.info(
      `[AIGateway] Available model types: ${availableModelTypes.join(", ")}`,
    );
    ...
    try {
      ...
      logger.info(
        `[AIGateway] Authentication configured: ${config.useOIDC ? "OIDC" : "API Key"}`,
      );
      logger.info("[AIGateway] Plugin initialization complete");
      logger.info("[AIGateway] ✅ Features enabled:");
      logger.info("[AIGateway]   • OpenAI-compatible REST API (/v1/*)");
      logger.info("[AIGateway]   • Server-Sent Events streaming");
      logger.info("[AIGateway]   • Socket.IO streaming integration");
      logger.info("[AIGateway]   • Tool calls via ElizaOS Actions");
      logger.info("[AIGateway]   • File upload support");
      logger.info("[AIGateway]   • Dynamic model discovery");

      if (!config.grokModelsEnabled) {
        logger.info(
          "[AIGateway] 🚫 Grok models are blocked in support of ElizaOS",
        );
        logger.info(
          "[AIGateway] 🔄 Alternative models will be used automatically",
        );
      }
    } catch (error) {
      logger.error(
        "[AIGateway] Error registering model handlers:",
